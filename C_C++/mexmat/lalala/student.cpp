...
student::student(const student&x)//конструктор копирования
{
	init(x.name, x.value);
}

student::student(student&&x)//конструктор перемещения
{
	value=x.value;
	name=x.name;
	x.value=0;
...
student& student::operator=(const student&x)// оператор копирования
{
	destroy();
	init(x.name,x.value);
	return *this;
...
student& student::operator=(student&&x)// оператор присваивания
{
	destroy();
	value=x.value;
	name=x.name;
